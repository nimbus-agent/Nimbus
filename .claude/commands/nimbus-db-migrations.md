...
**Version numbers are strictly sequential — never reuse or skip a number.** Latest applied migration: **V59** — "multimodal remote-model grants", the `media_grant` table behind invariant `I37` / static `D27` (artifact-scoped, per-vendor remote-vision consent; `media-grant-store.ts` is its SOLE writer, `media-discovery.ts` holds one correlated `EXISTS` read, and `index/media-grant-v59-sql.ts` is the DDL — D27(b) confines the table name to exactly those three files). Recent predecessors: **V58** `media_pass_cursor` (the resumable multimodal understanding pass), **V57** `cu_session`/`cu_action` (the computer-use session + action stream, `I35`), **V56** `sync_state.configured` (never-set-up connectors — the signal `I29`'s `sync` class gates its append on), **V55** `pr_changed_file` + `pr_files_state`, **V54** `graph_entity.metadata` namespacing (no new table), **V53** the pre-mortem theme tables, **V52** the derived `item.resolve_key`, **V51** the ownership graph. **Derive the number from `INDEXED_SCHEMA_STEPS` in `migrations/runner.ts`, not from this sentence** — it sat at V55 through four migrations. [`docs/schema-reference.md`](../../docs/schema-reference.md) is the authoritative dated list.
