...
| **Local filesystem, no network (Tier-5)** | localdb, storybook, great-expectations | Pure path-traversal-guarded filesystem reads — no network, no DB connection, no code execution; the configured dir is added to `filesystem.read` at spawn (`phase3Add<X>Mcp`), the MCP server reads it from env. localdb (`localdb.scripts_dir`) indexes saved `.sql` files (`localdb:saved_query`); storybook (`storybook.dir`) reads the `index.json`/`stories.json` manifest (`storybook:story`); great-expectations reads validation-result JSON (see no-row-data row above). |
