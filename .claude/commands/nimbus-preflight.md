...
- **`audit:status-drift`** — keeps the doc "status surfaces" (CLAUDE.md, GEMINI.md, architecture.md, SECURITY-INVARIANTS.md) in sync with the canonical highest invariant (`I<N>`) and schema version (`V<N>`) read from code. Bumping an invariant/migration without updating the status lines fails this gate. It also derives three claims the docs make about code: the I13 write-route surface (count + full enumeration, now across `cli-reference.md`, `SECURITY-INVARIANTS.md` and the `nimbus-http-write-surface` / `nimbus-federation-identity` skills), the I7 `allowlist_exact_size`, and the I17 LAN-admitted set.
- **`audit:doc-refs`** — every `[text](path)` link and backtick path in CLAUDE/GEMINI, **all of `docs/`** and `.claude/{commands,agents}/*.md` must resolve on this branch. `DOCS_EXCLUDED_PREFIXES` in the script names the few docs deliberately out of scope (the dated CHANGELOG, the forward-looking roadmap, archived `superpowers/` plans, upstream-repo paths, generated audit artifacts) and why.
- **`audit:workflow-lint`** — parses every workflow, rejects a `run:` body line at column 0 (which silently invalidates the file: GitHub records a run with ZERO jobs and no log), `bash -n`s every bash body, and requires **`timeout-minutes` on every job that has `steps:`**. Without a cap a job inherits GitHub's 360-minute default, so a wedged step burns a six-hour runner slot; reusable-workflow callers (`uses:`) are skipped because GitHub rejects the key on them.
