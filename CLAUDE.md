...
- **CI-Linux-only failures — reproduce, don't guess:** CI is Ubuntu + Bun `1.3` (the `bun-version` default in `.github/actions/setup-nimbus-ci/action.yml`; no test/quality workflow overrides it — `bun-version: latest` appears only in `org-drift-sweep.yml`). Some failures never reproduce on Windows/macOS and aren't version-related — chiefly `mock.module` contamination in the combined `bun test packages/cli/src` run (prefer **dependency injection (DI) over `mock.module`** for dispatcher-driven code) and `@types/*` hoisting conflicts. Reproduce on Linux (Docker `oven/bun:latest`, or WSL on a Linux-native copy — not `/mnt/c`) **before** pushing a fix. `audit:coverage-floor` is **CI-Linux-authoritative**. Details: `nimbus-preflight` skill.
