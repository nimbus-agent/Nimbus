...
describe("mapLaunchDarklyFlagToItem", () => {
  test("returns null when the row is not a plain object", () => {
    expect(
      mapLaunchDarklyFlagToItem(null, { baseUrl: BASE, projectKey: "default", syncedAt: NOW }),
    ).toBeNull();
    expect(
      mapLaunchDarklyFlagToItem("nope", { baseUrl: BASE, projectKey: "default", syncedAt: NOW }),
    ).toBeNull();
  });
  ...
  test("returns null when key is missing or empty", () => {
    ...
    expect(
      mapLaunchDarklyFlagToItem(makeFlag({ key: "" }), {
        baseUrl: BASE,
        projectKey: "default",
        syncedAt: NOW,
      }),
  ...
  test("title from name; falls back to key", () => {
    const withName = mapLaunchDarklyFlagToItem(makeFlag(), {
      baseUrl: BASE,
      projectKey: "default",
      syncedAt: NOW,
    });
  ...
  test("kind accepts boolean/multivariate; unknown → null", () => {
    for (const k of ["boolean", "multivariate"]) {
      const row = mapLaunchDarklyFlagToItem(makeFlag({ kind: k }), {
        baseUrl: BASE,
        projectKey: "default",
        syncedAt: NOW,
      });
    ...
    const garbage = mapLaunchDarklyFlagToItem(makeFlag({ kind: "rollout" }), {
      baseUrl: BASE,
      projectKey: "default",
      syncedAt: NOW,
    });
  ...
  test("modifiedAt = max env lastModified; falls back to creationDate then syncedAt", () => {
    const withEnvs = mapLaunchDarklyFlagToItem(makeFlag(), {
      baseUrl: BASE,
      projectKey: "default",
      syncedAt: NOW,
    });
    ...
    const createdOnly = mapLaunchDarklyFlagToItem(noEnvMod, {
      baseUrl: BASE,
      projectKey: "default",
      syncedAt: NOW,
    });
  ...
  test("url === canonicalUrl and points at the project flag page", () => {
    ...
    expect(row.canonicalUrl).toBe(
      "https://app.launchdarkly.com/projects/default/flags/enable-new-checkout",
    );
