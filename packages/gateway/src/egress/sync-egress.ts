 ...
 * appends nothing there), `multimodal/cloud-bytes.ts`'s `fetchCloudBytes` (one row per FETCH
 ...
 * request), and `ipc/connector-rpc-handlers/auth.ts`'s `verifyBeforeStoreWithScopes` (one row,
 * `method='connector.credentialProbe'`, destination = the service id, before the pre-store
 * credential probe that `nimbus connector auth` — and local-auth adoption — sends to
 * github/gitlab/bitbucket/jira/jenkins; a service with no probe makes no request and appends
 * nothing). The cloud-bytes pair is the reason a single Photos/OneDrive candidate produces TWO
 * rows: it makes two real outbound requests, and one row covering both would have meant a
 * candidate that failed at RESOLVE left no row at all for a request that had already gone out.
 ...
 * `TargetedFetchDeps`. The next two seams are wired together by
 ...
 * to the byte fetch alike). The fifth is called directly from `verifyBeforeStoreWithScopes` (or its
 * injected `appendProbeEgress` test seam). None of the five callers imports the raw
 ...
 * fetch appends one row for its one call, each cloud resolve/byte-fetch appends one row per
 ...
 *
 * Called BEFORE the outbound call by all five callers; throwing here aborts the caller's
 ...
 * any of the five seams without a wrapping arrow function hiding an accidental async leak.
 */
