...
/**
 * Firebase App Distribution authentication.
 *
 * App Distribution's REST API is a Google Cloud API: it accepts a short-lived
 * OAuth2 access token in the `Authorization: Bearer` header. We mint that token
 * from a Google **service account** key (the JSON the developer downloads) using
 * the JWT-bearer grant — no `googleapis` dependency.
 *
 * The flow:
 *   1. Build an RS256 JWT asserting the service account's identity, scoped to
 *      `cloud-platform`, with `aud` = the SA's `token_uri`.
 *   2. POST it to the token endpoint with
 *      `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer`.
 *   3. Receive `{ access_token, expires_in }` and send it as a Bearer token.
 *
...
export function signServiceAccountAssertion(
  ...
  // (do NOT use ieee-p1363 — that is for EC keys only). Pass the digest name
  // explicitly ("sha256") rather than `null` so the signer works on Bun's
