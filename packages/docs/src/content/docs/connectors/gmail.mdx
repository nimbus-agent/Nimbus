...
- **Service id:** `gmail`
- **Item types:** `email`
- **Body:** the message body, up to **16 KiB**. The sync walks the MIME tree of a `format=full`
...
Fetching bodies uses `format=full` instead of `format=metadata`. That is the same Gmail API request
at the same 5-quota-unit cost, but it returns the inline part bytes — so an initial mailbox sync
...
`summary` clamps the body to 512 characters; `metadata_only` suppresses it entirely. See
