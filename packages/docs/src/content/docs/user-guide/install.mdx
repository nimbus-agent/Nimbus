...
the gateway, indexes the repo, and — when the index found one — prints a real
`file:line` from your own code to try with `nimbus why` and, in a terminal, offers
to run `nimbus wow` right away; a repo with nothing to show yet gets `nimbus wow`
as a next step instead of the offer. It appends to your config rather than
rewriting it, so existing settings and comments survive (and it keeps a `nimbus.toml.bak`).
