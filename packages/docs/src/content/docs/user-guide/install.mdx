...
to try with `nimbus why`, and, in a terminal, offers to run `nimbus wow` right
away. It appends to your config rather than rewriting it, so existing settings
and comments survive (and it keeps a `nimbus.toml.bak`).
