...
- **Allowed:** `external_id` (`<report_label>:<row key or index>`) · `report_label` ·
  the row's fields as generic metadata.
- The admin owns the report's field choices; as defense-in-depth a small **denylist
  heuristic** drops row keys whose names match `ssn`/`salary`/`comp`/`dob`/`birth`
  (case-insensitive).
...
2. **Time-off** — walk the Absence collection, same paging discipline; optionally bounded to
   a recent window to avoid unbounded history (default: forward + recent, e.g. last 365 days).
...
4. `oauthProfileForService` switch — add the `workday` OAuth profile (token endpoint shape).
