...
Superseded before implementation. The repo's own roadmap already documented (2026-06-03) that Chromatic's public token-auth GraphQL API exposes only `Project.lastBuild` — no paginated builds-list field reachable from a headless token — which invalidates the design's per-project builds walk. Decision: drop Chromatic and close Wave B docs-only (App Center cancelled — upstream retired; Chromatic deferred). No connector code was written; these review points were therefore not actioned in code. See the 2026-06-04 entry in `docs/CHANGELOG.md`.
