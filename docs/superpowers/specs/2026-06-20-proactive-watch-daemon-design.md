...
- **`watch-daemon.ts`** — `WatchDaemon` class. Lifecycle (`start()`/`stop()` + `AbortController`), injected `now: () => number` and `clock`/`sleep` seams (DI for tests, mirroring `auto-update.ts`). Holds three triggers:
  1. **Daily briefing tick** — once per day at the configured `briefing_hour`, resolved in the configured `[watch].timezone` (falling back to the OS local TZ when unset); computes `nextFireAt` and persists it to `watch_daemon_config`. The explicit `[watch].timezone` override exists so a Gateway running on a UTC remote VM still fires the daily briefing in the user's local timezone rather than UTC.
...
5. **Platform equality** ✅ — pure Bun event loop + SQLite + the cross-platform `broadcastNotification`. No OS-specific service, no per-OS code. `briefing_hour` resolves in the configured `[watch].timezone` (IANA name) when set, else the OS local TZ — identical resolution on every platform.
...
7. **CLI noun overload (`nimbus watch`).** The incumbent connector-watcher subsystem already owns `nimbus watch list|pause|resume` (→ `watcher.*`). **This spec does NOT rename the shipped `nimbus watch` connector-watcher command** — it keeps both subsystems under the one `nimbus watch` noun (additive subcommands) and keeps the specced RPC split intact (new `watch.*` for the proactive daemon vs incumbent `watcher.*` for the connector-watchers), since the daemon *is* "the thing that watches" and the namespaces stay distinct regardless.
   ...
   **OWNER DECISION (recorded, not adopted here):** a reviewer proposed instead moving the incumbent connector-watcher CLI to a new noun — `nimbus hook` / `nimbus events` — to free `nimbus watch` for the proactive daemon. This is left as an explicit owner call and is **NOT** taken in this spec because it is a **BREAKING change to a shipped CLI surface** (`nimbus watch list|pause|resume` is already public): adopting it requires a deprecation/alias path (keep `nimbus watch <old>` working with a deprecation warning that redirects to `nimbus hook <old>` for at least one release) before the old noun is removed. Until the owner decides and that alias/deprecation plan is designed, the additive single-noun approach above ships as-is.
