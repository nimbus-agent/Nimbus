...
**Not already shipped (verified).** There is no `egress_ledger` table and no `egress/` directory. The migration list in `packages/gateway/src/index/migrations/runner.ts` ends at `simpleStep(42, 43, …)` (V43 `share_inbox`); **V44 is unused**. The closest shipped neighbors are reusable but distinct:
...
**Invariant impact — new invariant I29** (I28 is reserved for the unmerged MCP-server owner-sink on `dev/asafgolombek/phase7-mcp-gateway-server`; this is the next free number):
...
> - **Static complement:** `scripts/structure-audit/check-nimbus-invariants.ts` (new check `D22`): the egress-ledger append symbol and the `egress_ledger` table-write are confined to `egress/*`; no other file may INSERT into `egress_ledger`; `executor.ts` must reference the egress sink.
...
2. **Read-action coverage.** Today only HITL-class *writes* clearly pass `gate()` as `PlannedAction`s; do *read* connector dispatches (the bulk of "did anything leave?") also flow through `execute()`? If some read dispatches bypass `ToolExecutor`, the "zero egress on this query" claim has a hole. **Needs a one-pass audit of every `connectors.dispatch()` call site** before shipping — the completeness claim is only as good as the chokepoint's totality. This is the single most important pre-implementation verification.
