...
2. **AGPL distribution obligation messaging:** the embedding guide must state that bundling/shipping the `nimbus-gateway` binary triggers AGPL source-availability for that binary. Confirm wording with the maintainers' commercial-license stance (`docs/roadmap.md:46`).
...
- [ ] `packages/docs` gains an "Embed Nimbus (powered by Nimbus)" guide covering the spawn lifecycle, the loopback/no-egress defaults, the HITL relay contract, and the AGPL-binary distribution obligation.
