...
| `packages/cli/src/tui/App.tsx` (363 L) | 57.6% | Extend the existing 155-line test using `ink-testing-library`. Drive state transitions: idle → streaming → awaiting-hitl → disconnected. The TUI state machine `tui/state.ts` is already covered; this is about exercising the surface that reacts to it. |
...
| `packages/gateway/src/extensions/install-from-local.ts` (808 L) | 66.9% | Extend the existing test if present (likely is, given the 66.9% measurement). Target the audit-on-rejection paths, the `completeExtensionInstallAfterCopy` failure modes, and the signature-mismatch branches. ~6-8 cases. **Read the source first** to confirm which branches are uncovered; the cases below are guesses. |

### Tier D — Phase 4 hard one (retry)

| File | Baseline | Approach |
|---|---|---|
| `packages/gateway/src/embedding/model.ts` (64 L) | 13.51% | Phase 4 hit a `mock.module("@xenova/transformers", ...)` collision because `embedding/create-routing-runtime.test.ts` mocks the same module path. Bun's `mock.module` is process-global. Fix: write the test into a separate file path that bun's test runner spawns as its own process. The simplest mechanism is putting the test under `packages/gateway/test/integration/embedding/model-isolated.test.ts` — `bun test:integration` uses a separate runner invocation and the mock state does not bleed in. Alternative: extract the dynamic `await import("@xenova/transformers")` call into its own tiny module (`embedding/load-transformer-pipeline.ts`) that `model.ts` imports as a function. Then `model.ts` is pure helpers and the new tiny shim can be structurally excluded. **Choose the test-isolation approach first; fall back to the extract-and-exclude refactor if isolation proves fragile.** Final fallback per spec rule 3 is raised watermark. |
...
| 12 | `test(embedding): cover model.ts via isolated test process (Tier D retry)` | 1 file | ~4 (or partial + raised watermark) |
...
**Totals:** ~49 new tests across ~12 test files + 2 exclusion entries + 6 stale baseline housekeeping + final baseline drop.
...
| Separate-process test file isolation | `embedding/model.ts` (commit 12) |
| Tmp-dir + real fs ops | `extensions/install-from-local.ts` (commit 11), `ipc/server/socket-listeners.ts` (commit 9) |

**Test file locations:** colocated next to source file per Phase 3/4 precedent — `<name>.test.ts` next to `<name>.ts`. The single exception is `embedding/model.test.ts` (commit 12), which is intentionally placed under `packages/gateway/test/integration/embedding/` so bun's `test:integration` runner gives it a separate process and the `mock.module` global doesn't collide with `create-routing-runtime.test.ts`.
...
| `socket-listeners.ts` Windows-only functions (`startWin32NetServer`, `attachWin32Socket`) measured as 0% on Ubuntu CI even though the implementation is cross-platform | The functions use plain `net` module APIs (`net.createServer`, `net.Socket`) which work cross-platform. The function names imply Windows but the code does not. Tests can connect a real client to a `net.createServer()` on Linux — verify in the implementer report. If it turns out platform-specific behavior is genuinely involved (named pipes), split the file: extract the cross-platform pure-handler logic to `socket-listeners.ts`, leave a tiny `win32-listener.ts` shim, exclude the shim. |
...
| `embedding/model.ts` isolation fallback (commit 12) needs `bun test:integration` to be a genuinely separate process | Verify before implementation by running `bun run test:integration` from `main` and confirming separate-process semantics. If not, fall back to the extract-and-exclude refactor described in the Tier D table. Final fallback: partial + raised watermark. |
