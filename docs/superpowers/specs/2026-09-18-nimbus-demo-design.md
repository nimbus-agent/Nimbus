...
| socket | macOS/Linux: `<same dir as the real socket>/nimbus-gateway-demo-<h>.sock`. Windows: `\\.\pipe\nimbus-gateway-demo-<h>`, where `<h>` is the first 12 hex chars of SHA-256 over the demo root path, on EVERY OS — a named pipe is MACHINE-global, and a unix socket directory (`TMPDIR`/`XDG_RUNTIME_DIR`) can be shared by two demo roots, so a fixed name would let a test booted on temp roots connect to a developer's live demo gateway (§ 10; the unix half shipped in #1545's review round) |
...
1. **Structural:** a demo gateway's `SyncScheduler` is constructed but NEVER STARTED, has NO
   syncable registered, and is built with `syncDisabled` so `forceSync`/`tick`/`pump` refuse
   whoever calls them (superseded wording "does not construct" — see § 11.2 for why). No connector
   can sync into the demo index, whatever credentials or config reach it.
...
## 11. PR 1 outcome and pre-plan spikes for PR 2 (2026-09-19)

**PR 1 shipped as #1545 (`8791c199`).** Beyond § 10, its reviews added: the Linux gateway resolves
paths BEFORE its keyring probe and skips the probe when demo-rooted (a 4th host-global leak); every
CLI "not running" hint goes through ONE builder (`cli/src/lib/gateway-not-running.ts`) guarded by a
static test; the clip-pairing URL is gated by the boot policy and `nimbus --demo serve` refuses; the
unix demo socket is hashed too (§ 3.1 table); I41 clause (1) carries one stated exception — an
explicitly exported `NIMBUS_GATEWAY_LOG_PATH` on a DIRECT gateway launch (`nimbus --demo start`
always overrides it).

### 11.1 § 2.8 answered — `nimbus --demo ask` (live probe on temp roots, Ollama not running)

On the EMPTY demo index `ask` makes NO model call: it takes the `empty_index` route and prints
"No data indexed yet … nimbus connector auth github / google / slack … nimbus connector sync
<service>". Two consequences for PR 2:
1. Those hints drop `--demo` AND name commands a demo gateway refuses. PR 2 makes the empty-index
   hint demo-aware (in demo mode it points at `nimbus demo`, which always seeds).
2. The SEEDED path (`local_context` / `agent_tools`, which needs a model) is observable only once the
   corpus exists — it stays an e2e capture task (§ 6), with the rule unchanged: an unhandled error or
   a hang there is fixed in `ask` for every fresh install, never special-cased for demo.

The probe also re-confirmed isolation under real use (start/status/ask/stop on the hashed pipe, no
leftover process) and that the only files written outside the demo root were Bun's own transpiler
cache under HOME and a `bun-node-*` shim in TEMP — nothing Nimbus-owned.

### 11.2 § 2.9 answered — the sync scheduler (code survey, verified)

A demo gateway cannot simply skip `syncScheduler.start()`: `tick()` and `pump()` return early only
on `stopped`, never on `!started` (`sync/scheduler.ts` ~456/526), `forceSync` (~312) needs no start,
and every finished job's `.finally` calls `tick()` — so ONE `connector.sync` or `index.rebody` on an
unstarted scheduler runs a job and then re-animates the whole schedule with no timer. NOT constructing
it is also wrong: `createSchedulerWithMesh` builds the connector mesh and the
glossary/decisions/premortem/ownership refreshers alongside it, and ~15 type sites assume it exists.
The status reads § 2.9 worried about (`connector.listStatus`/`status`/`healthHistory`, `diag`,
heartbeat, shutdown) read the DB or tolerate an idle scheduler — none need a started one.

**PR 2 design (supersedes § 5 layer 1's "does not construct"):** in a demo gateway —
1. construct the scheduler; do NOT call `start()` (this also removes `probeConnectivity()`, a network
   probe) — a new `bootPolicy.syncScheduler` field, pinned by the I41 test like the other two;
2. register NO syncables (skip `registerFilesystemRootSyncables`, `registerConnectorMeshSyncables`,
   `registerUserMcpSyncablesFromDatabase`) — with nothing registered, `runJob` fails "Unknown
   service", `syncableFor` disables targeted fetch, and no demo `scheduler_state` rows appear;
3. refuse `connector.addMcp` (the only runtime `register`) — already inside § 5's `connector.*`
   allow-list refusal;
4. ONE choke point in the scheduler itself: a `syncDisabled` constructor option under which
   `forceSync` rejects and `tick()`/`pump()` no-op — so a future caller that bypasses the IPC refusal
   still cannot run a sync. Keyed on the option, not on `!started` (existing tests call `forceSync`
   without `start()`).
Consequence for the corpus: the demo's `[[filesystem.roots]]` is NOT synced by a filesystem
syncable; `why` needs the files only to exist on disk (`matchConfiguredRoot`'s `existsSync`), and
blame/commit rows are seeded directly.

### 11.3 Carried from PR 1 into PR 2's scope
- A demo gateway still makes outbound calls: the updater's `checkOnStartup` (default `true`) and
  env-selected telemetry (`NIMBUS_TELEMETRY_ENABLED`) — the demo `nimbus.toml` disables the updater
  and a demo gateway ignores the telemetry env, so § 4.2's "nothing that makes an outbound call"
  becomes true, with a test that proves it (no egress row, no updater fetch).
- `nimbus --demo doctor` reports the REAL OS vault's health, which the demo never uses — it must
  report the in-memory vault instead.
- Follow-up hints that drop `--demo` (`admin console`'s `nimbus vault get …`, onboarding's
  `nimbus connector auth …`, and § 11.1's empty-index hint) — a pasted hint must not run against the
  real install.
- `mcp/tool-runtime.ts`'s mid-connection-drop hint is not demo-aware.
- The static not-running guard's regex misses the "start it with: nimbus start" variant — widen it.
