...
Setup needs a `uses: bencherdev/bencher@<pinned-sha>` install step (SHA-pinned per the repo's Scorecard pin policy, like the g-a-b action). The `BENCHER_API_KEY` secret is exposed only to this perf job. The `benchmark:` job also needs **`checks: write`** added to its `permissions` block — `bencher run --github-actions` posts the advisory **"Bencher Report" check run** (the §5.5 PR-side output under `--ci-only-thresholds`, and the commit check on push). It is the least-privilege addition, narrower than the `contents: write` / `pull-requests: write` the job already holds.
