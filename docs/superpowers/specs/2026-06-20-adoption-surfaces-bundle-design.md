...
- **Reuse (already shipped — NO change, NOT novel work):**
  ...
  - `packages/gateway/src/db/audit-verify.ts` `verifyAuditChain` — the BLAKE3 verify (already behind `audit.verify`); the new summary calls it to attach `chainIntact`.
...
- **To add (CLI):** one verb `nimbus audit policy` in the **existing** `packages/cli/src/commands/audit.ts` (a `runAuditPolicy` calling `audit.complianceSummary`). `nimbus audit list|verify|export` already ship.
- **To add (UI):** a policy-ceiling header in the **existing** `AuditPanel.tsx` consuming `audit.complianceSummary`; no new RCE-class exposure.
...
- **Invariant impact:** **Reuses I22** (read `EnforcedPolicy`, never raw policy TOML), **respects I2/I4** (renders `hitlStatus` set only by the consent gate; never sets it). **No new invariant** — the slice is read-only and adds no defense to the I1–I27 set (the current ceiling per `docs/SECURITY-INVARIANTS.md`, whose I27 row states "count stays I1–I27"). If a *future* (out-of-scope) export path ever needed a new structural defense, it would take the next free number **I28**; the SECURITY-INVARIANTS.md "Worked example" uses `I28` only illustratively (sub-agent tool scope), so I28 is unclaimed, not reserved. (The unmerged MCP-server owner-sink on `dev/asafgolombek/phase7-mcp-gateway-server` would, if it lands, claim its own next-free number at merge time — this slice does not depend on or encroach on it.)
...
## Open questions
1. **Surface home:** Both surfaces already exist (the `AuditPanel.tsx` Tauri panel and the `nimbus audit` CLI), so the question is *which to extend first* with the new policy-fusion read. Recommendation: extend the **CLI** first (`nimbus audit policy`, ships immediately on every platform), then add the policy header to `AuditPanel.tsx` as a fast-follow (Phase 13 desktop vehicle is the broader distribution gate).
...
- [ ] New `nimbus audit policy` CLI verb lands in the existing `packages/cli/src/commands/audit.ts`; `nimbus audit list|verify|export` continue to work unchanged.
- [ ] No new invariant (the set stays I1–I27; the next free number remains I28 and is unclaimed); no migration. All 7 Non-Negotiables verified in the PR description against the wiring sites named above.
