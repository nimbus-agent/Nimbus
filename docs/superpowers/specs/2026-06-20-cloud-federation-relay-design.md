...
1. **Local-first (machine is source of truth):** PRESERVED. The relay carries *opaque sealed blobs and key-hash metadata only* — it never holds plaintext, keys, or answers, and is fully optional (mesh degrades to direct P2P). The source of truth remains each machine; the relay is "a connector for reachability," not a data store. Metadata minimization is structural: salted key-*hashes* not pubkeys, no query logging, TTL-expiry, no who-talks-to-whom persistence required by the protocol.
   ...
   **Privacy disclosures (v1 stable-hash linkability — be honest about the residual leak):** the v1 routing tag is a *per-recipient-stable* salted key-hash so the relay can mailbox-route. While the relay can read no bodies, real pubkeys, or method names, a relay operator observing traffic over time **can map per-recipient network topology**: which mailbox (recipient tag) is drawn on by which sender tags, and thus the who-talks-to-whom communication graph (plus timing/size/volume). This is the deliberate v1 trade-off — stable tags are what make a dumb store-and-forward mailbox work without the relay learning the real key registry. **Mitigation commitment:** a subsequent slice replaces the stable hash with **Ephemeral Routing Keys (ERKs)** — short-lived routing tags deterministically derived from the existing pairing key, rotated per epoch and agreed offline at pairing time, so both peers compute the same tag without the relay ever learning the stable linkage (folds into the Phase 22 "unexfiltratable" refinement). v1 ships the stable hash with this disclosure; v2 ships ERKs.
...
- **I31 enforcement test (if invariant approved):** assert no real pubkey / method name / body string ever appears in a posted envelope; assert relay-client confinement (D24 static) — added to `security-invariants.test.ts`.
...
- **No V47 persistence in v1** unless cross-restart queueing is proven necessary (Phase 13.5 line 1696 explicitly chooses "no queueing").
...
3. **Metadata-minimization vs. relay efficiency:** *RESOLVED.* Ship the per-recipient-stable salted key-hash for v1 (the relay already sees timing/size, and a stable hash is what makes mailbox routing work) — **but record the privacy cost explicitly** (see "Privacy disclosures" in §Security) and commit to rotating routing tags in a subsequent slice. The stable hash lets the relay operator map per-recipient *network topology* (which mailbox draws traffic from which senders, and the who-talks-to-whom graph over time) even though it can read no bodies. v2 replaces the stable hash with **Ephemeral Routing Keys (ERKs)** — short-lived routing tags deterministically derived from the existing pairing key (rotated per epoch, agreed offline at pairing time so both peers compute the same tag without the relay learning the linkage) — folding into the Phase 22 "unexfiltratable" refinement.
...
- If I31 is approved: wiring + `docs/SECURITY-INVARIANTS.md` row + `security-invariants.test.ts` test + static `D24` in `scripts/structure-audit/check-nimbus-invariants.ts` all land in the same commit; I28 left reserved for the MCP-server branch.
