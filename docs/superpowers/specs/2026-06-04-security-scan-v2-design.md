...
- **Windows arg-length safety (plan review #1):** the per-file excerpt ranges are coalesced (`mergeRanges`) and, past a disjoint-range cap (`MAX_BLAME_RANGES` = 64), the connector issues one **full-file** blame instead of hundreds of `-L` args — keeping the command line well under the Windows `CreateProcess` 32 767-char limit (Platform-equality #5). The `MAX_BLAME_LINES` per-file skip bounds the fallback's storage.
