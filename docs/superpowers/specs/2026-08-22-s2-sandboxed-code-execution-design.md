...
6. **`nimbus exec` does NOT work on Windows, and this document should not be read as claiming it does.** Linux is fully verified — 5/5 against a real `bwrap` sandbox, loopback included. On Windows the AppContainer helper itself is fine (the pre-existing `sandbox-wrapper-spawn` suite passes 5/5 there with a `powershell.exe` child), and after fixes 4 and 5 the child now *starts* — but `bun` then dies with `CouldntReadCurrentDirectory` even though the helper grants cwd `FILE_GENERIC_READ|EXECUTE|WRITE`. Granting ancestors is not a workaround: it reproduces the documented ACE-explosion hang. The remaining fix plausibly lives in the native helper or in choosing a different Windows child strategy, which is scope beyond this slice. **The integration suite is deliberately left failing on Windows rather than skipped**, because a skip and a pass are indistinguishable and this is exactly the gap that convention exists to expose.
