...
- `packages/browser-extension/` (new, **MIT** — it is a client like `packages/cli` and `packages/vscode-extension`, reaches the gateway over the local HTTP surface only, imports no gateway source). Manifest V3 (Chromium) + a Firefox-compatible manifest. Background service worker holds the bearer token (stored in `chrome.storage.local`); a toolbar action + context-menu "Save selection to Nimbus" builds the payload from `tabs`/`scripting` APIs (title, URL, `window.getSelection()`), POSTs to `http://127.0.0.1:<port>/v1/web/capture`. CSP/manifest blocks all external hosts — the only `host_permissions` entry is `http://127.0.0.1/*` (loopback). No external CDN, no analytics.

**CLI (AGPL)**

- `packages/cli` (extend): `nimbus web token set|show|clear` to provision the `web_capture.bearer` Vault secret and print the localhost capture URL for paste-into-extension setup. `nimbus web enable|disable` toggles `[web_capture].enabled`.
        ...
        --> http-server.fetch (127.0.0.1 bind, I6)
            --> dispatchWriteRoute (I13): resolve route -> requireBearer (I10 constant-time)
                ...
                -> runWebCaptureRoute -> dispatchWebCaptureRpc
                   -> mapWebPageToItem (validate, clip, hash URL -> externalId)
...
- **No new executor action / HITL frozen-set entry** — unless the owner picks the review-queue model (Open Questions).

## Open questions

1. **HITL posture:** ship as a direct bearer-authed index write (recommended — deliberate per-page user click, no cloud side-effect), or add a `web.capture` executor action + a review queue so the owner approves before indexing? The recommended path keeps the executor gate untouched; the alternative adds a frozen-set entry (would be a same-commit triple-rule change).
...
3. **Extension ↔ gateway pairing UX:** manual paste of the `nimbus web token show` value (simplest, recommended), or a short-lived pairing-code handshake (Phase-6 pattern) to avoid copy-paste?
...
- [ ] A valid bearer-authed capture writes exactly one `web:page` item (re-capture upserts), schedules its embedding, and writes an audit entry containing the token **fingerprint** and URL host but never the token or the secret.
