...
- `packages/gateway/src/connectors/web-page-mapping.ts` (new): `mapWebPageToItem(raw, ctx): MappedRow<"web", "page"> | null` — the Mendeley-mapper template. Title clipped to 120, snippet/selection clipped, `externalId` = a deterministic hash of the normalized URL (so re-capturing the same page upserts, not duplicates). `metadata = { source: "web_clipper", url, title, snippet, selection?, capturedAt, browserContext, extensionVersion }`. The `source: "web_clipper"` tag is filterable in `nimbus search` and distinguishes clipped pages from connector-synced items.
