...
This keeps the Wave 5 acceptance criterion honest by SCOPING it rather than overstating it: when a
local LLM is configured for the pass, the consolidated definition is generated by it with zero live
API calls; when none is, the definition is a labelled verbatim snippet. The criterion is not "every
consolidated definition is LLM-generated" — that would be false on the path the scheduler actually
takes today.
...
`--refresh` and `--rebuild` are **not implemented**. They were designed to force a pass immediately
and to truncate both tables, delete projected items and re-mine from a zero watermark
(`rebuildGlossary` exists in the gateway but has no caller) — but no IPC path reaches either, so
the CLI rejects both flags with an explicit error rather than running an ordinary query while the
user believes something else happened. See Known Limits. Extraction is driven solely by the
...
| E2E (subprocess) | `cli/test/e2e/glossary.smoke.e2e.test.ts` | real Bun process + fresh temp dir: no-gateway exit code, `--rebuild` rejection, `help` listing |
...
- **`--refresh` and `--rebuild` are rejected, not honoured.** The gateway's `agents.glossary`
  handler reads only `term` and `limit`, so neither flag ever triggered a pass. Found during the
  Task 15 review, 2026-07-30. They were originally parsed and forwarded, which meant
  `nimbus glossary --rebuild` printed an ordinary listing while the user believed the glossary had
  been re-derived from scratch; the CLI now fails with an explicit "not implemented yet" error
  instead (2026-07-31, CodeRabbit review of PR #981). Until they are wired, extraction is driven
  solely by the post-sync trigger. Wiring them is a follow-up.
...
- [ ] When a local LLM is configured for the pass, the consolidated definition is generated by it
      with **zero live API calls**; without one the definition is a verbatim snippet, labelled as
      such (§5.7). The scheduler-triggered pass supplies no LLM today, so it takes the snippet path.
