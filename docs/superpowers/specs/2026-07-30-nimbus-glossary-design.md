...
nimbus glossary --refresh  # run an on-demand pass now, then print the (possibly updated) brief
nimbus glossary --rebuild  # re-derive every term from scratch (destructive; requires --yes)
```
...
CREATE TABLE IF NOT EXISTS glossary_pass_state (
  ...
  watermark_ms  INTEGER NOT NULL DEFAULT 0,   -- modified_at of the last row scanned
  watermark_id  TEXT    NOT NULL DEFAULT '',  -- item.id tiebreaker within that timestamp
...
WHERE item_fts MATCH ? AND (i.service || ':' || i.type) IN (...)
```

The filter is **service-qualified**, not a bare `i.type IN (...)`. `message`, `page`, `issue` and
`commit` are generic type names several services share, so a bare-type filter silently admits every
out-of-scope source reusing one — `wiz:issue` today, and any user-installed extension emitting
`message` tomorrow. It also inflates `service_spread`, which is a geometric multiplier in the score.
Corrected 2026-07-31 (CodeRabbit review of PR #981; independently found by the whole-branch review).
...
1. Read the `(watermark_ms, watermark_id)` cursor; select source-scoped items ordered by
   `(modified_at, id)` and strictly past it. The `id` half is load-bearing: with a
   timestamp-only cursor, a batch truncated by `SCAN_BATCH_LIMIT` inside a group of rows sharing
   one `modified_at` — ordinary after a bulk import — would advance past the whole group and skip
   its remainder permanently.
...
8. Advance the cursor to the `(modified_at, id)` of the LAST row scanned; record pass stats. **Commit.**
...
is simply picked up by the next pass's phase-B batch selection. Deferring the watermark until after consolidation —
...
- **An automatic upgrade path exists.** `consolidatePhase` runs a second batch each pass —
  `selectSnippetUpgradeBatch()` selects `status='consolidated' AND definition_source='snippet'`
  rows, backed off by the same retry-cooldown shape as pending terms — and re-consolidates them
  through the LLM when one is configured (`[glossary].use_llm`, default true). The upgrade batch is
  guaranteed a reserved floor of slots (`UPGRADE_RESERVE`, clamped to half the per-pass budget), so
  a large pending backlog can slow upgrades but never starve them indefinitely. A failed upgrade
  attempt leaves the existing snippet definition untouched — only `attempts` and `last_attempt_at`
  advance. Unlike snippet mode, the upgrade path CAN veto: a term a snippet pass let through with no
  judgment can be vetoed and removed from the glossary the first time a model actually evaluates it.
  Full allocation rule, ordering, and the vetoed-term reporting this drives in `--refresh` output:
  §3.2 of the LLM-wiring follow-up spec (`2026-07-31-nimbus-glossary-llm-wiring-design.md`).
- The brief always labels which mode produced a definition, so a raw quote is never mistaken for
  a synthesized definition.

This keeps the Wave 5 acceptance criterion honest: when a local LLM is configured for the pass
(`[glossary].use_llm = true`, the default) and one is actually available, the consolidated
definition is generated by it with zero live API calls; when no LLM is configured or available, the
definition is a labelled verbatim snippet — no longer a permanent state, since the upgrade path
above re-consolidates it once a model becomes available.
...
bounded local-LLM time, already capped at 25 calls per pass. `use_llm` is a separate knob from
`enabled`: `enabled = false` disables the whole feature, while `use_llm = false` keeps mining and
the snippet-definition path running but never spends a model call.

`--refresh` and `--rebuild` are implemented, through a dedicated `glossary.*` IPC namespace rather
than by extending `agents.glossary`'s parameters — see §9. `--refresh` runs an on-demand pass
immediately, sharing the single-flight guard with the debounced scheduled path. `--rebuild`
truncates both `glossary_term` and `glossary_pass_state`, deletes every projected item, and
re-mines from a zero watermark (`rebuildGlossary`); without `--yes` it only previews the count and
sample of what would be deleted. Full CLI/IPC design: the LLM-wiring follow-up spec
(`2026-07-31-nimbus-glossary-llm-wiring-design.md`) §4. Extraction is otherwise still driven by the
debounced post-sync trigger.

## 8. The agent (read path)

`runGlossary` fans out **two** `AgentCoordinator` lanes per invocation — two per mode, four across
both modes. Each lane reads a different table, so the parallelism is real rather than ceremonial:
...
Corrected 2026-07-30 after the Task 13 review. The original design called for four lanes per
invocation, but the resolved term already carries `top_sources` and `synonyms`, so dedicated lanes
for them re-ran the same lookup and discarded the result — burning coordinator budget while making
...
- **IPC:** `agents.glossary { term?, limit? }` → `{ sessionId }`, followed by
  `glossary.briefReady` / `glossary.briefError`, for the read path. `--refresh` / `--rebuild` do
  **not** extend those parameters — they are driven by a separate `glossary.*` namespace
  (`glossary.refresh` / `glossary.rebuild` → `{ jobId }`), long-running jobs emitting
  `glossary.passProgress` / `glossary.passDone` / `glossary.passError`. Both methods are
  LAN-forbidden (I5) and not Tauri-exposed (I7) — see the LLM-wiring follow-up spec
  (`2026-07-31-nimbus-glossary-llm-wiring-design.md`) §4.
- **Tauri:** `agents.glossary` added to `ALLOWED_METHODS` (**I7**) — 101 → 102, including the
  ...
  `glossary.refresh` / `glossary.rebuild` are write-class and local/CLI-only, so they were not
  added — the count stays at 102.
...
| Agent | `agents/glossary.test.ts` | 2 lanes per mode, exact→synonym→near-miss order, gap notes, both modes |
| CLI | `commands/glossary.test.ts` | arg parsing, render, exit codes — **DI, not `mock.module`** |
| E2E (in-process) | `gateway/test/e2e/scenarios/glossary.e2e.test.ts` | brief sections, `briefReady` emitted, frequency ranking, **zero HITL fires**, zero `egress_ledger` rows |
| E2E (subprocess) | `cli/test/e2e/glossary.smoke.e2e.test.ts` | real Bun process + fresh temp dir: no-gateway exit code, `--rebuild` preview without a gateway, `help` listing |
...
- **The abort signal does not reach the LLM provider.** `GlossaryRefresher.stop()` aborts the
  ...
  both `OllamaProvider` and `LlamaCppProvider` hardcode their own 120s timeout. Shutdown stops
  *waiting* on an in-flight model call; it does not cancel the in-flight HTTP request, which keeps
  running until that timeout or process exit. Bounded by there being at most one abort per process,
  at shutdown. See the LLM-wiring follow-up spec (`2026-07-31-nimbus-glossary-llm-wiring-design.md`)
  §2.3 / §7.
- **Vetoes are sticky, with one reset.** A vetoed term (`status='vetoed'`) is never re-asked by the
  ordinary pending or upgrade batches. `nimbus glossary --rebuild --yes` is the only path back to
  re-evaluating it — it truncates the term along with everything else and re-mines it from a zero
...
- [ ] When a local LLM is configured for the pass (`[glossary].use_llm = true`, the default) and
      available, the consolidated definition is generated by it with **zero live API calls**;
      without one the definition is a verbatim snippet, labelled as such (§5.7), and upgraded to an
      LLM-consolidated one on a later pass once a model becomes available.
