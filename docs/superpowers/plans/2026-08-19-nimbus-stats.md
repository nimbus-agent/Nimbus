...
- **The two NEW counters bucket on a real event timestamp** — `pr-merges` on `metadata.merged_at`, `incidents-opened` on `metadata.opened_at_ms`. Neither may bucket on `modified_at` or `synced_at`; `item` has no creation timestamp, so those measure last activity or our own sync schedule (spec F1/D1). **The four WRAPPED DORA metrics inherit `dora.ts`'s `item.modified_at` predicate**, which is effectively event time for the `ci_run`/`deployment` rows behind `deployment-frequency` but genuine last-touch for the `pr` and `incident` rows `lead-time`, `change-failure-rate` and `mttr` read; `mttr` additionally inherits DORA's `synced_at` fallback. That is the accepted cost of calling four tested calculators unchanged rather than reimplementing them — see spec § 3 D1. An earlier revision of this line asserted the property for all six, which was false.
