...
- **Every authorized outbound action is ledgered.** An append-only, BLAKE3-chained egress ledger records what left the machine, and `nimbus prove` reports it. The structural rules behind all of this are enumerated as invariants `I1`–`I31` in [`SECURITY-INVARIANTS.md`](./SECURITY-INVARIANTS.md); each has a production wiring site *and* an enforcement test.
...
- **31 enumerated invariants** — `I1`–`I31` (`I28` reserved), each with a production wiring site, a section in [`SECURITY-INVARIANTS.md`](./SECURITY-INVARIANTS.md), and an enforcement test. A static audit runs before the test suite; the runtime tests stay authoritative.
