...
**Compliance recipe:** a new disclosure that is a whole section (like `## Gaps`) goes in `RESERVED_HEADINGS_BY_KIND` and is built by `reservedBlocksFor`, then held back from the prompt via `omitReserved` and re-attached by `joinReserved` — never rendered into the synthesizable body. A new disclosure that is interleaved within a section that itself must reach the model (a confidence ceiling inside prose, a truncation count inside a findings list) cannot be held back the same way; it goes through the Layer 2 anchor-phrase mechanism in `brief-contract.ts` (`requiredPhrases`/`contractViolations`), which checks the phrase survived rather than constructing it — PR 2's scope for the thirteen kinds not yet covered there.
