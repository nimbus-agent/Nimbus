...
None of these are exploited today; the point is that the static check cannot see them, so it must not be described as though it can. Closing them is capability removal (Phase 2 of the I29 security spec), not a stronger regex. Separately: this phase adds **no new egress coverage** — the coverage vector (`packages/gateway/src/egress/egress-coverage.ts`) records that `task` is `per-call` and every other class (`session`, `sync`, `model`, `peer`) is `none`; the vector, not this prose, is the machine-readable claim, and it is carried in the HASHED `source_id` of a per-process boot marker so it cannot be edited without breaking the chain.

**Wired at:**

- `packages/gateway/src/engine/executor.ts` `ToolExecutor.gate()` — the egress row is appended (via the injected `EgressSink`, a REQUIRED constructor parameter with a named `NULL_EGRESS_SINK` for the gate-only executors that never dispatch) after the audit record and before the `if (hitlStatus === "rejected")` return, so both approved and denied decisions are ledgered. If `EgressSink.append` throws, `gate()` throws, and `execute()` propagates the error before reaching `connectors.dispatch` (fail-closed).
...
- `packages/gateway/src/egress/egress-source-type.ts` — the FROZEN 8-member `source_type` union (`task`/`prune`/`session`/`sync`/`model`/`peer`/`boot`/`degraded`); it is an input to `computeEgressRowHash`, so widening it later is a chain break, not a refactor. Also defines `MARKER_SOURCE_TYPES`/`isMarkerSourceType`, which exclude the three bookkeeping classes from the outbound count.
- `packages/gateway/src/egress/egress-coverage.ts` + `egress-boot-marker.ts` — the per-process coverage claim: `THIS_BINARY_COVERAGE` states what this binary is built to observe, and `appendBootMarker` writes it (serialized into `source_id`) once per process so a build that never wires a sink produces a boot marker claiming nothing, rather than a silent, indistinguishable-from-real empty ledger.
...
- Enforced statically by **D22** in `scripts/structure-audit/check-nimbus-invariants.ts` — (a) any non-test file other than `engine/executor.ts` that references the literal string `connectors.dispatch` causes `audit:invariants` to exit 1; (b) any non-test file outside `packages/gateway/src/egress/` that references `appendEgressEntry` causes `audit:invariants` to exit 1. Per the Known limits above, (a) is a confinement on that literal string, not a totality guarantee over every code path that can reach the network.
- Runtime test in `packages/gateway/src/security-invariants.test.ts` — the `I29` describe block: `egress.prune` ∈ `HITL_REQUIRED`, append-before-dispatch ordering, blocked row on deny, abort on append failure, the D22 presence assertion, the D22 comment-scope assertion (does not claim totality; does describe the literal-string mechanism), the required-sink assertion, and `NULL_EGRESS_SINK` vs `makeEgressSink` writing to a real ledger.
