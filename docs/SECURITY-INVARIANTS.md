...
- `packages/gateway/src/ipc/agents-rpc.ts` `dispatchAgentsRpc` — the MCP brief chokepoint: when `ctx.caller.kind === "mcp"` and the method is in the `agents.` namespace, `recordMcpBriefEgress(ctx.db, …)` runs BEFORE `dispatchByMethod`, so the row exists before any agent work starts. No `try/catch` — a throwing append propagates and no brief is emitted (fail-closed, mirroring `executor.ts` `gate()`). `ctx.caller` is server-derived at connect time (`session.declareKind`), never taken from the RPC params.
