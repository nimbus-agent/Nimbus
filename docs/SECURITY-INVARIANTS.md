...
**Wired at:** every MCP / extension spawn site in `packages/gateway/src/connectors/lazy-mesh/` — 78 as of 2026-08-16, each setting `env: extensionProcessEnv(...)`. Distributed across `phase3-config.ts` (45, the largest block), `connector-spawns.ts` (31 per-connector spawns), `chatops-bot-spawn.ts` (2), `mesh.ts` (filesystem MCP in the constructor) and `user-mcp.ts` (`ensureUserMcpClient`). Do not hand-maintain these counts against the tree: the previous version of this line said "30+ … `connector-spawns.ts` (16), `phase3-config.ts` (8)" and omitted `chatops-bot-spawn.ts` entirely, and the enforcement test's floor of 20 had drifted to less than a quarter of the real figure. The test now derives the set instead of counting toward a number.
