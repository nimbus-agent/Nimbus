...
Rule (c) has the analogous limit, in the opposite direction: it pins the single CALLER of `recordMcpBriefEgress`, so it catches a second file acquiring the appender — but a second entry point that serves a brief WITHOUT calling the appender spells nothing it matches. A new surface that calls the `agents/*` `emit*Brief` functions directly (a browser-reachable agent route and a resolve-by-URL read are both recorded as wanted; see `docs/roadmap.md` § Track 2 → Client surfaces) would append nothing, serve the brief, and leave `audit:invariants` green. Nothing in the static check would notice, because the check's subject is the appender, not the brief. Closing that is the same answer as above — route every such surface through one dispatch entry point and append there before any work — not a stronger regex.
...
- `packages/gateway/src/egress/mcp-brief-egress.ts` `recordMcpBriefEgress` — the sole appender for that path. `destination` is `"mcp"`, or `"mcp+federation"` for the peer-querying agents (`agents.ghost`, `agents.huddle`); `source_id` is the client id; `hitl_status='not_required'` (the agents surface is read-only and reaches no HITL-gated action) and `result_status='authorized'`.
...
- Enforced statically by **D22** in `scripts/structure-audit/check-nimbus-invariants.ts` — (a) any non-test file other than `engine/executor.ts` that references the literal string `connectors.dispatch` causes `audit:invariants` to exit 1; (b) any non-test file outside `packages/gateway/src/egress/` that references `appendEgressEntry` causes `audit:invariants` to exit 1; (c) any non-test file other than `ipc/agents-rpc.ts` (the single caller) and `egress/mcp-brief-egress.ts` (the definition) that references `recordMcpBriefEgress` causes `audit:invariants` to exit 1. Per the Known limits above, (a) is a confinement on that literal string, not a totality guarantee over every code path that can reach the network. Rule (c) pins the caller rather than merely permitting an appender, so adding a file to it would satisfy the checker while dissolving the property — it is not an allowlist to extend.
- Runtime test in `packages/gateway/src/security-invariants.test.ts` — the `I29` describe block: `egress.prune` ∈ `HITL_REQUIRED`, append-before-dispatch ordering, blocked row on deny, abort on append failure, the D22 presence assertion (all three rule names), the D22 comment-scope assertion (does not claim totality; does describe the literal-string mechanism), the required-sink assertion, `NULL_EGRESS_SINK` vs `makeEgressSink` writing to a real ledger, the source scan proving `recordMcpBriefEgress` is named by exactly two production files, the `COVERAGE_CLASSES` ≡ non-marker-source-types identity, and the "every class claiming non-`none` has a landed appender" assertion (`mcp` + `task`). Behavioural coverage of the appender itself lives in `packages/gateway/src/egress/mcp-brief-egress.test.ts` and the `I29` block of `packages/gateway/src/ipc/agents-rpc.test.ts` (CLI and callerless dispatches append nothing; an MCP dispatch appends exactly one row; a failed append emits no brief).
...
**How to comply:** every new clipper-token path routes through `verifyApiToken` (constant-time) and only mints after `PairingWindowController.confirm` returns a label plus its recorded scopes; a new scope-gated route gets an entry in `HTTP_ROUTE_AUTH` and reads its requirement via `clipScopeFor`, never a hardcoded `hasScope(scopes, "…")`.
