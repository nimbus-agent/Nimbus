...
- `packages/gateway/src/egress/agent-brief-egress.ts` `recordAgentBriefEgress` — the sole appender for that path. `destination` is `"mcp"`, or `"mcp+federation"` for the peer-querying agents (`agents.ghost`, `agents.huddle`); `source_id` is the client id; `hitl_status='not_required'` (the agents surface is read-only and reaches no HITL-gated action) and `result_status='authorized'`.
