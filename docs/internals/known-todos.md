...
- `packages/gateway/src/connectors/lazy-mesh/user-mcp.ts:34` (2026-05-28): Thread the registry-stored user-MCP manifest into `MeshSpawnContext` (as `ctx.lookupUserManifest`) and replace `USER_MCP_DEFAULT_MANIFEST` with that lookup — Task 15 + post-PR 1 follow-up. Until then the conservative default-deny sandbox manifest is applied to all user MCPs regardless of their declared permissions.
