...
- **Notification contract** — each agent emits exactly one `<agentName>.briefReady { sessionId, brief: string, synthesis }` IPC notification on completion. `brief` is always Markdown; `synthesis` is the `SynthesisProvenance` union (`agents/_lib/synthesize.ts`) naming whether `brief` is the deterministic render or an LLM-synthesized rewrite, and why not when it isn't (see the `nimbus-agent-patterns` skill for the full shape). This field list is not exhaustive — a `findings` field carrying the agent's own typed brief object also rides this notification; out of scope for this correction.
