...
| S1 | `pre-mortem` | `nimbus pre-mortem <epic-ref> [--service <name>]… [--json] [--refresh] [--repropose]` | `agents.premortem` (read, Tauri-exposed, HTTP/MCP-excluded); `premortem.refresh` (on-demand pass, LAN-forbidden, not Tauri-exposed) | ✅ Shipped 2026-08-10 — the thirteenth built-in agent and PR B of the S1 pre-mortem work (PR A shipped schema **V53** + the background theme pass 2026-08-09): four sequential lanes resolve a Jira epic to its affected services, build an IDF-weighted service-overlap cohort of comparable closed epics, compute five structural risks (cycle-time overrun, size overrun, review drag, incident coupling, deploy-failure), and read recurring blocker themes from V53. **The one built-in agent that is not purely read-only**: it proposes — via the narrowly-bounded exception in `nimbus-agent-patterns` — one paused (`enabled = 0`) `incident_opened` watcher per affected service, never armed by the agent itself. **No deploy-failure watcher is ever proposed** (annotate provider slug vs. syncable service id mismatch); review drag is unmeasurable for most repos (no connector indexes a PR's opened timestamp); Jira-only, and `parent_key`-derived cohort membership is team-managed-Jira-only — no `linear:project` items are indexed at all. Confidence ceiling 0.86, matching `glossary`/`decisions`. Excluded from the HTTP + MCP agent surfaces (matching `agents.preflight`) since its writes have no HITL gate; emits `agents.premortem.briefReady` |
...
// 2026-08-09; PR B: the reader + cohort/risk/watcher logic, 2026-08-10):
...
//   overrun, review drag, incident coupling, deploy-failure), and `themesForServices` recurring
...
//   writes one `watcher` row per affected service, always `enabled = 0` (paused), plus its
//   `premortem_watcher_proposal` tombstone, in one transaction so the two can never land
//   separately. This is NOT an I2/HITL matter: I2 governs `HITL_REQUIRED_BACKING` action types
