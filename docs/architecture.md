...
**Status:** Phase 4 ✅ Complete · Phase 5 (Extended Surface) ✅ Complete (2026-06-04) — T1–T6 ✅ · Wave A ✅ · Wave B ✅ · Tiers 1–5 ✅ (remaining connectors are documented non-gating deferrals; see [`roadmap.md`](./roadmap.md)) · Phase 6 (Team) 🚧 In progress (Slices 1–5 shipped; Slice 6a delivered 2026-06-11; Slices 6b + 6c delivered 2026-06-12; current invariants through I25; schema V39). `v0.1.0` released 2026-05-09 (headless Gateway + CLI + VS Code extension; `desktop-v0.1.0` Tauri release deferred to Phase 13). Dated delivery log: [`CHANGELOG.md`](./CHANGELOG.md). Workstream-level status is in [`roadmap.md`](./roadmap.md).
...
| HTTP write surface (`I13`) | `ipc/http-server.ts` | dedicated read-write handle; `WRITE_ROUTE_ALLOWLIST` (5 routes) — `POST /v1/deployments` + the three SCIM `/scim/v2/Users` routes + `PUT /v1/admin/policy` (Phase 6 Slice 4 signed-policy install) |
