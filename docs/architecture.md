...
//   asking "is there something in THIS index worth showing, and with what arguments?" against the
//   real substrate — never a demo-only path. `steps` (default 3) is refused, never clamped,
...
// scan and registers by hand instead, proven only by its own test. See "Active invariants summary"
// below for D31 alongside D30.
```
