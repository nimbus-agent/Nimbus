...
**Status + dated delivery log:** see [`CHANGELOG.md`](./CHANGELOG.md) (canonical) and [`roadmap.md`](./roadmap.md) (phases + acceptance criteria). Current invariants through I35 (I28 reserved); schema V57.
...
//   or when the requested lane is absent from `allowed_lanes`; asserts `SandboxRunner.canConfine`
//   for the lane's policy (never `degradedReason()`/`isFullyActive()`, I33's identical reasoning);
//   then the LOCAL owner approves the verbatim envelope — lane, the full `navigateOrigins` +
...
//
// **Honesty bound, stated where the methods are documented rather than only in the invariant
// prose:** the browser DRIVER does not exist in this build. `playwright-core` failed a
// `bun build --compile` gate (a statically-resolved `require` inside its bundled, unused
...
// `wrapLedgeredBrowserContext`, a decorator over the driven `BrowserContext`) is consequently
...
The canonical invariant table (currently **I1–I35**, with I28 reserved) lives in [`SECURITY-INVARIANTS.md`](./SECURITY-INVARIANTS.md) — each row names the defense, its production wiring site, the anti-pattern that regresses it, and the enforcement test. It is deliberately **not** duplicated here: a third copy (alongside the compact summaries in `CLAUDE.md` / `GEMINI.md`) is how it drifts. When changing a wiring site, update the invariants file *and* the enforcement test in the same commit.
