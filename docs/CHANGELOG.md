...
- **2026-09-17 — the `nimbus media allow-remote` consent preview shows real sizes.** The defect the
  ...
  media source: `index.queryItems` sets an item's top-level `sizeBytes` only from a `size_bytes`
  metadata key, and no media connector writes that key — `filesystem` writes `sizeBytes`, Google
  Drive and OneDrive write `size` — so all three reached the wire with the size buried in
  `rawMeta`, which the CLI never reads. The acceptance run saw it on Drive only because Drive was
  the only source it previewed. The CLI's own tests could not catch it: their fixtures put
  `sizeBytes` at the top level, a shape the gateway never sent for a media row. The media pass
  itself was unaffected, since `media-discovery.ts` reads raw metadata through
  `mediaSourceBytes`. The fix fills `sizeBytes` in the `index.queryItems` handler from that same
  table, so the preview and the pass's byte budget read one definition; a hydrated `size_bytes`
  still wins, and Google Photos, which records no byte count, still reads `size unknown` rather
  than an estimate. Additive on the wire: `nimbus query --json` now also carries `sizeBytes` on
  filesystem media, Drive and OneDrive rows. No migration, no invariant, no egress class.
...
- **2026-09-06 — the multimodal remote arm was answered by a real vendor for the first time.**
  ...
  **Fixed 2026-09-17 (entry above), and the cause was wider than this sentence says:** every media
  source previewed as `size unknown`, not only Drive. Full record: `2026-09-02-s2-multimodal-io-design.md` §
  21.
