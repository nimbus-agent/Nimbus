...
- [ ] **GCP Cloud Logging** — the sibling of AWS CloudWatch Logs above (reuses `gcp.*` creds like BigQuery): log names, sinks, metric definitions metadata; `gcloud logging logs/sinks/metrics list`; **NEVER** `gcloud logging read` (log entries = row data); no-row-data contract test
