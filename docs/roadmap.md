...
- [x] **Agent personas (A2)** ✅ 2026-08-18 — the last of the three Wave 6 answer-quality surfaces, after A0 (2026-08-16) and A1 (2026-08-18, Wave 6 section below). A `[persona]` section in `nimbus.toml` — `tone` (`neutral` default / `terse` / `formal` / `casual` / `verbose`) and `voice` (`neutral` default / `opinionated` / `collective`) — shapes `nimbus ask` and every built-in agent brief. **`confidence_threshold` and `tool_caution` are rejected, not deferred:** `tool_caution` was already prohibited on 2026-08-16 (Non-Negotiable #2 + I2's frozen `HITL_REQUIRED_BACKING` set forbid a config knob that changes what triggers a consent prompt); `confidence_threshold` is rejected on the same reasoning one layer up — a dial that lets the agent hedge less is a dial that makes it sound more certain than it is, against every S1 honesty surface built this month (I31, the 0.86 confidence ceilings on `decisions`/`pre-mortem`, `negotiate`'s "could not be computed" instead of `0`). One definition — `engine/persona.ts`'s `TONE_DIRECTIVES`/`VOICE_DIRECTIVES` — applied at two sites: `nimbus ask`'s single site above the router-vs-agent fork (composing outward around `--devil`, persona outermost so the devil directive is never diluted), and `agents/_lib/synthesize.ts` via the same `buildAgentSynthesisRunner` factory both the socket (`ipc/server/dispatchers.ts`) and HTTP (`agent-runs/agent-http-invoke.ts`) brief paths already share. **D6** — no directive may govern *whether* content appears, only *how* — is enforced by a test over the directive constants pinned against an omission-verb pattern; it is what keeps `terse` coherent alongside `--devil` and what keeps it from pushing against **I31**, which needed no new guard: reserved sections are re-attached verbatim and never shown to the model, and interleaved disclosures are anchor-checked, so a terse persona's only measurable cost is a higher discard rate back to the deterministic render — now self-describing via the resolved persona riding the `briefReady` notification's `synthesis` field alongside the existing rejection reason. Two adjacent bugs fixed in the same branch: `[agents] synthesis` was profile-blind (`loadNimbusAgentsFromConfigDir` hardcodes `nimbus.toml`) and now reads from the same profile-resolved path as `[persona]`; and `ProfileManager` — declared as an optional dependency but never constructed in production — is now wired in `platform/assemble.ts`, which is the first time the desktop app's Profiles settings page and its four already-allowlisted `profile.*` IPC methods have worked rather than thrown. **Switching a profile still requires a Gateway restart** (`NIMBUS_PROFILE` is fixed at spawn); the panel now says so, matching what the CLI already printed. No schema migration, no new IPC method, no new HTTP route, no new invariant, no Tauri allowlist change — `ALLOWED_METHODS` stays at 105. Dated detail: [`docs/CHANGELOG.md`](./CHANGELOG.md).
