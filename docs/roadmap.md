...
- [x] **`nimbus pre-mortem <epic-ref>`** ✅ 2026-08-10 — built **narrower than this row's original scope** in three ways, all deliberate: **Jira-only**, not "Jira / Linear" — no `linear:project` items are indexed at all, so there is no Linear epic-shaped row to analyze; **proposes, never schedules** — it writes a PAUSED `incident_opened` watcher per affected service (plus its `premortem_watcher_proposal` tombstone) that the user must explicitly arm, rather than "proactively scheduling" a live alert; and **no cross-team mesh composition** — Phase 6 federation is not wired into this agent. What it does: resolves an epic to its affected services, builds an IDF-weighted service-overlap cohort of comparable closed epics from the local index, computes five structural risks (cycle-time overrun, size overrun, review drag, incident coupling, deploy-failure), and reads recurring blocker themes from the V53 background pass. **No deploy-failure watcher is ever proposed** — a deployment item's `item.service` is the annotate provider slug while the watcher engine matches syncable service ids, so a service-filtered deploy-failure watcher could never fire; the risk is still computed and reported, it simply proposes nothing. `agents.premortem` is excluded from the HTTP and MCP surfaces (matching `agents.preflight`) because it writes local rows with no HITL gate, but is Tauri-exposed (I7) since that sits behind the renderer's own XSS threat model. Emits `agents.premortem.briefReady`. Dated detail: [`docs/CHANGELOG.md`](./CHANGELOG.md).
