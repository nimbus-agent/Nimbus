...
- [ ] **Answer-quality surfaces, remaining** — first-class negation + aggregation queries (W6-B) (Phase 7 Wave 6), the last of the four Wave 6 answer-quality items now that brief synthesis (A0, 2026-08-16), devil's-advocate mode (A1, 2026-08-18) and agent personas (A2, 2026-08-18) have all shipped — see above. W6-B carries a 2026-08-16 correction on its Phase 7 Wave 6 row that changes its shape materially: "the structured index already handles negation natively" is true only of raw SQL, and there is no predicate language, so `--negate` means inventing one, not surfacing something that already exists.
