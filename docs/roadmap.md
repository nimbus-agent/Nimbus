...
- [ ] **I31 PR 2 — interleaved-disclosure phrase layer.** PR 1 (2026-08-17) closed whole-section disclosure integrity (`## Gaps` plus `negotiate`'s two reserved sections) structurally. What is left: `negotiate`'s seven null-lane sentences (`_could not be computed_`) move out of `render.ts` string literals into named constants in a new `agents/_lib/brief-disclosures.ts`, read by both the renderer and `brief-contract.ts`'s `requiredPhrases` so the two can no longer drift; and the anchor-phrase check widens to cover the interleaved lines PR 1 left unchecked — `negotiate`'s ownership list-truncation clause, its last-modified-not-created window clause, its two `unattributable` disambiguation lines, and `glossary`'s two definition-provenance lines. See `docs/superpowers/specs/2026-08-17-brief-honesty-contract-design.md` § Delivery.
