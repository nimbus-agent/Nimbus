...
- [ ] **Answer-quality surfaces, remaining** — agent-persona configuration (A2) and first-class negation + aggregation queries (W6-B) (Phase 7 Wave 6). Brief synthesis (A0) — the prerequisite question these implicitly assumed answered, whether an LLM is in the brief path at all — shipped 2026-08-16, and devil's-advocate mode (A1) shipped 2026-08-18 (`nimbus ask --devil`; one directive constant in `engine/devil-advocate.ts`, injected at the prompt chokepoint both execution paths share, wired across both `agent.invoke` and `engine.askStream`); see above. Both remaining items carry a 2026-08-16 correction on their Phase 7 Wave 6 rows that changes their shape materially: A2's profiles are whole-FILE swaps rather than a `[profile.<name>]` table, so there is no table to extend, and its `tool_caution` knob is **prohibited** as originally scoped (Non-Negotiable #2 + I2 forbid a config knob that loosens HITL); W6-B has no predicate language to surface, so `--negate` means inventing one.
...
- [ ] **Agent personas (A2)** — extends the Phase 3.5 configuration profiles with persona configuration per profile: `tone` (terse / formal / casual / verbose), `voice` (neutral / opinionated / first-person-plural), `confidence_threshold` (how often the agent volunteers "I'm not sure"), `tool_caution` (eager / measured / paranoid). [Corrected 2026-08-16, checked against the tree for the A0 design spec: **wrong shape** — `config/profiles.ts` implements profiles as whole-**file** swaps (`nimbus.<name>.toml` plus a `.nimbus-profile` marker), not a `[profile.<name>]` table, so there is no table to extend with a nested `.persona` block; whatever this becomes needs its own per-profile-file shape. Also **prohibited as originally scoped**: `tool_caution` cannot "affect HITL escalation defaults" — Non-Negotiable #2 ("consent gate... cannot be bypassed or configured away") and I2's frozen `HITL_REQUIRED_BACKING` set forbid a config knob that loosens HITL, so this wave cannot ship a caution level that changes what triggers a consent prompt.] Switching profiles via `nimbus profile switch <name>` already exists; this wave makes the persona configurable per profile so "work Asaf" can be terse + measured and "personal Asaf" can be verbose + opinionated.
