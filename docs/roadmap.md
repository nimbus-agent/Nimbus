...
- Wave 5: `nimbus glossary <term>` returns the team's actual usage of the term across at least 3 source threads with first-seen / last-seen dates ✅ 2026-07-30 (definitions are LLM-consolidated when a local LLM is configured; the scheduler-triggered background pass currently runs without one — `runGlossaryPass` is called with no `llm` from `platform/assemble.ts` — so unattended passes are snippet-sourced until the LLM is wired into that path, a follow-up rather than part of this slice)
