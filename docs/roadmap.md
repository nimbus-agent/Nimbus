...
| [`nimbus-web-clipper`](https://github.com/nimbus-agent/nimbus-web-clipper) | ✅ shipping (v0.2.0) | Its own roadmap, plus the clipper → **browser gateway client** reframe recorded below. Two open defects gate its *current* job first: #1005 (clip bodies truncated to 512 chars while `wordCount` reports the full length) and #1006 (`web_clip` routes to OpenAI embeddings when a key is set, contradicting the store listings' local-only claim); #1006 resolves before or with #1005 |
...
- **Two defects gate the clipper's current job and are not superseded by this direction:** #1005 (bodies truncated to 512 chars while `wordCount` reports the full length) and #1006 (`web_clip` routes to OpenAI embeddings when a key is set, contradicting the local-only claim in the store listings). #1006 must be resolved before or with #1005 — shipping a new surface on top of a truncated, silently-remote index would compound both.
