...
Two gateway surfaces are genuinely **not** built, and neither is a wiring job — state them plainly so the slice is not underestimated:

- **No browser-reachable way to invoke an agent.** `agents.*` is JSON-RPC over the local IPC socket; the bearer-authed HTTP surface the extension can speak has no agents route (`ipc/http-server.ts`). Its shape, scoping and auth are this repo's to design.
- **No resolve-by-URL read.** Nothing keys on `canonical_url`: `GET /v1/items` filters by service / type / time only, and `GET /v1/items/<id>` matches `id` or `external_id`. The column is written by the connector mappings and read by nothing — and it carries no SQL index (`item` is indexed on `service`, `type`, `modified_at` only), so the resolve read brings a migration with it, not just a handler.
