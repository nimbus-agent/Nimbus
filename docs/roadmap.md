...
- [ ] **Pocket** — saved articles, reading lists, tags; read-only index
...
- [ ] **Quorum HITL (the "two-man rule")** — extends Multi-user HITL: for the most destructive actions (e.g. `terraform destroy`, force-push to `main`, drop a production DB), the Team Vault requires *two* federated peers to approve within a bounded window before the credential unlocks. Enforced in the executor's HITL gate (`I2`), not the prompt — two-person control executed entirely over the local-first mesh with no central broker. The action-type → quorum-size mapping lives in the org-level policy engine.
...
- A federated query for an item type outside the peer's declared sharing scope returns empty and is recorded as a rejected query in the answering Gateway's audit log; revoking a peer's scope stops further answers within one sync cycle
- A quorum-gated action (e.g. `terraform destroy`) does not unlock its Team Vault credential until two distinct federated peers approve within the configured window; a single approval leaves the credential locked and logs the partial approval
