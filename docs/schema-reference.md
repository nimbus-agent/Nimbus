...
-- `browser` has a session gate wired in this delivery, and even that lane's driver is itself
-- deferred to raw CDP -- so a browser-lane session refuses with ERR_CU_NO_BROWSER once fully
-- configured -- the value set is known now, and widening a CHECK
