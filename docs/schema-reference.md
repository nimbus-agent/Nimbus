...
-- V63 additions (subject enumeration, spec § 6) below are sourced from
-- packages/gateway/src/index/fleet-subjects-v63-sql.ts.
--
...
--
-- V63 (spec § 6) adds the sweep cursor, nullable because a config-named job never sweeps:
--   sweep_kind            -- the SweepKind last enumerated under. If it differs from the job's
--     current configured kind, the cursor is treated as absent and overwritten -- a kind change
--     resets rotation rather than reading a stale cursor against a different subject space.
--   sweep_cursor          -- the last PROCESSED subject KEY, never an ordinal -- an ordinal shifts
--     when a subject is added or removed and would silently skip or repeat subjects.
--   sweep_subjects_total  -- the subject count at the last enumeration; reported by `fleet.list`.
--   sweep_empty_reason    -- the enumerator's reason when the last enumeration was empty, else
--     NULL; an accurately empty sweep is not a failure and must not back off.
-- Written only by `recordSweepEnumeration`/`advanceSweepCursor`, never by
-- `recordJobSuccess`/`recordJobFailure`, which use an explicit `ON CONFLICT ... DO UPDATE SET`
-- column list that leaves these four untouched.
CREATE TABLE IF NOT EXISTS fleet_job_state (
    ...
    sweep_kind            TEXT,
    sweep_cursor          TEXT,
    sweep_subjects_total  INTEGER,
    sweep_empty_reason    TEXT
) WITHOUT ROWID;
...
--     entire lifetime.
--   subjects_in_scope / subjects_attempted / subjects_completed -- V63 (spec § 6): a run's SUBJECT
--     accounting, distinct from the job counters above because a sweep job fans out into many
--     subjects per run. A config-named job counts as ONE subject in all three, so the
--     completed/attempted ratio means the same thing whatever mix of job kinds a run carried.
CREATE TABLE IF NOT EXISTS fleet_run (
    ...
    subjects_in_scope    INTEGER NOT NULL DEFAULT 0,
    subjects_attempted   INTEGER NOT NULL DEFAULT 0,
...
--
-- `subject_key` (V63, spec § 6) was added by REBUILDING this table -- SQLite cannot add a
-- `NOT NULL` column without a default, and a default would write a placeholder into history.
...
CREATE TABLE IF NOT EXISTS fleet_brief (
    ...
    job_id         TEXT NOT NULL,
    subject_key    TEXT NOT NULL,   -- V63; backfilled to job_id for every pre-V63 row
...
-- V61 -- saved (persisted) generated tools. Reserves invariant I40 and static rule D29(d) BY
-- NAME only -- this delivery (Task 2 of a 12-task branch) ships the table and its repository
-- module only; nothing writes a row yet. The save gate, the signature/schema verification, the
-- boot-time orphan sweep, and the load path that make I40 real are later tasks on this branch.
-- Authoritative source: packages/gateway/src/index/generated-tool-v61-sql.ts
--
-- The row governs EXISTENCE; disk plus signature govern CONTENT. A `saved/<toolId>` directory on
-- disk with no row here is an orphan and is swept at boot, never adopted, because a valid
-- signature proves an artifact was approved ONCE, not that it is approved NOW.
CREATE TABLE IF NOT EXISTS generated_tool (
    tool_id         TEXT PRIMARY KEY,
    tool_name       TEXT NOT NULL,
    description     TEXT NOT NULL,
    artifact_json   TEXT NOT NULL,   -- the canonical bytes that were signed, stored VERBATIM so
                                      -- verification never has to re-canonicalise
    artifact_digest TEXT NOT NULL,
    signature       TEXT NOT NULL,
    pubkey          TEXT NOT NULL,   -- PER ROW: a Vault key rotation then reads as the disclosed
                                      -- pubkey_rotated reason rather than masquerading as tamper
    approved_at     INTEGER NOT NULL,
    saved_at        INTEGER NOT NULL,
    last_loaded_at  INTEGER,
    disabled_reason TEXT             -- a SavedToolDisabledReason, or NULL when healthy
) WITHOUT ROWID;
