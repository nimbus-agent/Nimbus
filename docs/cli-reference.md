...
| `NIMBUS_CONFIG_DIR` | Override the platform config directory — **config only**. There is no data-directory override: the data directory is not relocatable by any `NIMBUS_*` variable except `NIMBUS_DEMO=1`, which moves EVERY path into the isolated demo subtree `<dataDir>/demo` (invariant I41) rather than repointing the real one (on Linux the real data directory follows `XDG_DATA_HOME`). |
