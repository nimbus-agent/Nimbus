...
removed — `git log --diff-filter=D -- docs/superpowers/` finds the prune commits).
...
markdown report into `docs/structure-audit/punchlist/` (creating the directory
if absent) and print only a one-line summary; only `strip-comments.ts` rewrites
source files in place. **That output is generated, not committed** — it sits
beside the other on-demand audit artefacts (`file-loc.json`, `knip-report.json`,
…) and is git-ignored, which is also why it must not be written back into a docs
tree the link and markdown gates walk. Run each from the repo root:
...
|---|---|---|
| `survey-comments.ts` | Flags load-bearing vs. noise comments across `packages/` + `scripts/` | Writes `docs/structure-audit/punchlist/01-load-bearing-comments.md` |
| `survey-oc.ts` | Finds Open/Closed-principle violations via a TypeScript AST walk (`if`/`switch` over literal unions) | Writes `docs/structure-audit/punchlist/04-oc-violations.md` |
| `survey-srp.ts` | Flags Single-Responsibility offenders (oversized modules / mixed concerns) | Writes `docs/structure-audit/punchlist/03-srp-offenders.md` |
| `survey-shape-dupes.ts` | Finds structurally duplicated object/type shapes | Writes `docs/structure-audit/punchlist/02b-shape-dupes.md` |
