...
export const CREDENTIAL_REGISTRY: readonly CredentialEntry[] = [
  ...
  },
  {
    ...
    maxAgeDays: 180,
    hardDeadline: null,
    note: "RETIREABLE once open-vsx.org upgrades — re-checked 2026-09-12, and the previous note here (`no OIDC path at all`) is now FALSE. eclipse-openvsx/openvsx#1534 closed COMPLETED 2026-08-21 and Trusted Publishing shipped in server v1.2.0 (PR #2000, commit d5a01c83) plus `ovsx` CLI v1.2.0; publish.yml already grants the `id-token: write` it needs. BLOCKED on DEPLOYMENT rather than upstream: https://open-vsx.org/api/version reports v1.1.2 — the release immediately before the feature (v1.1.2 shipped 2026-08-20, the merge landed 2026-08-21) — so the trusted-publisher registration UI does not exist on the instance we publish to, and switching would fail the next release instead of removing a secret. The trigger is a one-line check rather than more research: `curl -s https://open-vsx.org/api/version` reporting v1.2.0 or later. Migration steps and the precedence trap (a set OVSX_PAT overrides trusted publishing, so the secret must be REMOVED for the switch to take effect, or it looks done while nothing changed) are in docs/credential-hygiene.md. Rotation on the 180-day age policy until then. When it IS retired, this entry must move to state: forbidden / consumedBy: [] / maxAgeDays: null in the same change that deletes the secret - a deleted secret left at state: required is reported by auditCredentials as a hard missing every week, trading one recurring alert for another. Ordered runbook (the workflow hard-fails on an empty OVSX_PAT, so it needs a controlled-publish mode before the secret can go, and nimbus-vscode's bun.lock pins ovsx 1.1.0, which has no --trusted-publishing flag, so that dependency must be upgraded to >= 1.2.0 first): docs/credential-hygiene.md.",
  },
